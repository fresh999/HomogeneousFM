...
from data_utils import inf_train_gen, sl2_noise, sl2_section, sl2_to_lie_algebra_params, so3_noise, so3_section, so3_to_lie_algebra_params, stereo_inverse
...
def train(cfg: DictConfig) -> None:
    ...
    for i in range(cfg.training.iterations):
        ...
        # extrapolate Lie algebra parameters
        if mode == 'sl2':
            p_1 = sl2_to_lie_algebra_params(x_1)
            p_0 = sl2_to_lie_algebra_params(x_0)
        elif mode == 'so3':
            p_1 = so3_to_lie_algebra_params(x_1)
            p_0 = so3_to_lie_algebra_params(x_0)

        path_sample = path.sample(x_0=p_0, x_1=p_1, t=t)
        ...
        if (i+1) % cfg.training.log_every == 0:
            ...
            torch.save({
                ...
                'loss': loss
            }, os.path.join(cfg.training.output_dir, f'reduced_lie_{mode}_checkpoint_iter_{i}.pt'))
